        ...
        <div class="col-8">
            ...
            <div class="form-group">
                ...
                <img src="{{ asset('/images/'.$product->image) }}" style="width:150px"
                    class="rounded mt-2 border border-dark p-2">
            </div>
            <div class="form-group">
            ...
            <div class="card p-3">
                <div class="form-group">
                    ...
                    <select name="brand_id" id="" class="form-control">
                        @foreach ($brand as $d )
                        <option value="{{ $d->id }}" @if($d->id == $product->brand_id)
                            selected
                            @endif
                            >{{ $d->name }}</option>
                        @endforeach
                ...
                <div class="form-group">
                    ...
                    <select multiple id="category" name="category_id[]" id="" class="form-control">
                        ...
                        <option value="{{ $d->id }}" @foreach($product->category as $pc)
                            @if($pc->id==$d->id)
                            selected
                            @endif
                            @endforeach
                ...
                <div class="form-group">
                    ...
                    <select multiple id="color" name="color_id[]" id="" class="form-control">
                        @foreach ($color as $d )
                        <option value="{{ $d->id }}" @foreach($product->color as $pc)
                            @if($pc->id==$d->id)
                            selected
                            @endif
                            @endforeach
